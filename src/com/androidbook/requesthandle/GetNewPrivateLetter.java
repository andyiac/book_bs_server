...
public class GetNewPrivateLetter implements HandleRequest{
	...
	public int handleRequest(long uid, String password, String... param) {
		...
		try {
			...
			if(conn != null){
				...
				ResultSet rs = UserLetter.query(stat, UserLetter.TABLE_NAME, uid, UserLetter.NEW_LETTER_ID);
				if(rs.next()){
					String[] letters = rs.getString(UserLetter.NEW_LETTER_ID).split("#");
					...
					UserLetter.blankField(uid, stat, UserLetter.TABLE_NAME, UserLetter.NEW_LETTER_ID);
				}
			...
			e.printStackTrace();
			System.out.println("获取新私信出错： " + e.toString());
		} finally {
			try {
				conn.close();
